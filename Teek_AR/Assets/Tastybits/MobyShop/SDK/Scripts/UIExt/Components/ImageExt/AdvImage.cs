...
namespace UnityEngine.UI.MobyShop {
	...
	public class AdvImage : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
	{
		public float borderPctLeft = 0.05f;
		public float borderPctRight = 0.05f;
		public float borderPctTop = 0.05f;
		public float borderPctBottom = 0.05f;


		public bool borderGivenInPct=false;
		...
		// Scale the corners of the sprite.
		private void GenerateSliced2Sprite(VertexHelper toFill)
		{
			//Debug.Log("asdasd");
			if (!hasBorder)
			{
			...
			if (overrideSprite != null)
			{
				...
				border = overrideSprite.border;
				//KRM
				var dd = this.GetDrawingDimensions(false);
				//Debug.Log(" dd=" + dd );
				float width =  1; 
				float height = 1f;
			
				if( dd.x <= 0 ) {
					if( dd.w >= 0f ) { // normal case
						width = Mathf.Abs(dd.x) + Mathf.Abs(dd.z);
					} else { // pivot is outside box
						width = Mathf.Abs(dd.x) + dd.z;
					}
				} else {
					width = dd.z - dd.x;
				}
				...
				if( dd.y <= 0 ) {
					if( dd.w >= 0f ) { // normal case
						height = Mathf.Abs(dd.y) + Mathf.Abs(dd.w);
					} else { // pivot is outside box
						height = Mathf.Abs(dd.y) + dd.w;
					}
				} else {
					height = dd.w - dd.y;
				}
				...
				//var this_rt = this.GetComponent<RectTransform>();
				//this_rt.pivot;

				if (width < 0)
					Debug.LogWarning("Warning width is less than zero");
				if (height < 0)
					Debug.LogWarning("Warning height is less than zero");

				///GetDimensions
				border.x = height *(this.borderPctLeft/(borderGivenInPct?100f:1f));
				border.z = height *(this.borderPctRight/(borderGivenInPct?100f:1f)); // height*this.borderPctRight; 

				border.w = height *(this.borderPctTop/(borderGivenInPct?100f:1f));
				border.y = height *(this.borderPctBottom/(borderGivenInPct?100f:1f)); // height*this.borderPctRight; 
